...
def main():
    ...
    while running:
        ...
        isHumanTurn = (gs.whiteToMove and whitePlayer) or (
            not gs.whiteToMove and blackPlayer
        )
        ...
        for e in p.event.get():
            ...
            if e.type == p.QUIT:
                ...
                if not gameOver:
                    ...
                    if (
                        len(playerClicks) == 2 and isHumanTurn
                    ):  # if a user has made their second click, update the board and clear playerClicks
                        ...
                        for i in range(len(validMoves)):
                            if (
                                moveAttempt == validMoves[i]
                            ):  # if move is in all moves, make move, change moveMade variable, clear playerClicks.
                        ...
                        if (
                            not moveMade
                        ):  # if len(playerClicks == 2) but move not a valid move, clear playerClicks
                            sqSelected = ()
                            playerClicks = []
                            print(playerClicks)
