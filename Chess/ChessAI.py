    ...
    for row in range(len(gs.board)):
        for col in range(len(gs.board[row])):
            ...
            if square != "--":
                ...
                if color == "w":
                    score += pieceScore[piece] + piecePositionScore * 0.1
                elif color == "b":
                    score -= pieceScore[piece] + piecePositionScore * 0.1
    return score
...
def findBestMove(gs, validMoves, returnQueue):
    ...
    bestScore = findMoveNegaMaxAlphaBeta(
        gs,
        validMoves,
        depth,
        -CHECKMATE,
        ...
        True if gs.whiteToMove else False,
    )  # alpha = current max, so start lowest;  beta = current min so start hightest
    endTime = time.time()
    print(
        f"movesSearched: {counter}     maxScore: {bestScore:.3f}     Time: {endTime - startTime:.2f}"
    )
...
def moveSortAlgo(move, gameState):
    ...
    if move.pieceCaptured == "--" and gameState.squareUnderAttack(
...
def findMoveNegaMaxAlphaBeta(
    gs, validMoves, depth, alpha, beta, turnMultiplier, whiteAI
):
    global nextMove, counter, WhiteDepth, BlackDepth
    counter += 1
    if depth == 0:
    ...
    for move in validMoves:
        ...
        score = -findMoveNegaMaxAlphaBeta(
            gs, nextMoves, depth - 1, -beta, -alpha, -turnMultiplier, whiteAI
        )  # switch the alpha beta perspective.
        if score > maxScore:
            maxScore = score
            if (depth == WhiteDepth and whiteAI) or (
                depth == BlackDepth and not whiteAI
            ):
                nextMove = move
