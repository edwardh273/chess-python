...
class GameState:
    ...
    def getAllPossibleMoves(self):
        ...
        for r in range(len(self.board)):  # number of rows
            for c in range(len(self.board[r])):  # number of cols in given row
                ...
                if (color == "b" and self.whiteToMove == False) or (
                    color == "w" and self.whiteToMove == True
            ...
            if len(self.checks) == 1:  # only 1 check: block check or move king
                ...
                if (
                    ...
                    for i in range(1, 8):
                        ...
                        if (
                            validSquare[0] == checkCol and validSquare[1] == checkRow
                ...
                if 0 <= endRow < 8 and 0 <= endCol < 8:
                    ...
                    # check for checks
                        ...
                        if (
                            ...
                            if (
                                possiblePin == ()
                            ):  # if enemyPiece in range and no pin, inCheck = True.
                                inCheck = True
    ...
    def undoMove(self):
        if len(self.moveLog) != 0:  # make sure that there is a move to undo
            ...
            self.currentCastlingRights = CastleRights(
                lastCastleRights.wks,
                lastCastleRights.bks,
                lastCastleRights.wqs,
                lastCastleRights.bqs,
    ...
    def getCastleMoves(self, r, c, moves):
        if not self.whiteToMove and self.blackKingLocation != (4, 0):
            return
        if self.whiteToMove and self.whiteKingLocation != (4, 7):
            return
        if not (
            self.board[r][c - 1] == "--"
    ...
    def getPawnMoves(self, r, c, moves):
        ...
        if self.whiteToMove:  # white pawn moves
            ...
            if c - 1 >= 0:  # capturing left (ensures not off board)
                ...
                elif (c - 1, r - 1) == self.enpassantPossible and self.board[r][c - 1][
                    0
                ] == "b":
                    moves.append(
                        Move((c, r), (c - 1, r - 1), self.board, isEnpassantMove=True)
                    )
                    if not piecePinned or pinDirection == (-1, -1):
                        moves.append(
                            Move(
                                (c, r), (c - 1, r - 1), self.board, isEnpassantMove=True
                            )
                        )
